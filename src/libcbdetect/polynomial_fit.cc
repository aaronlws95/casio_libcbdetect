...
void polynomial_fit_saddle(const cv::Mat& img, const std::vector<int>& rs, Corner& corners) {
...
void polynomial_fit_monkey_saddle(const cv::Mat& img, const std::vector<int>& rs, Corner& corners) {
  // maximum iterations and precision
  int max_iteration = 5;
  double eps        = 0.001;
  int width         = img.cols;
